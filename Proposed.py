...
def create_quarter_points(center, radius, dim, r):
    points = []
    for a in np.linspace(0, 1, 4):
        new_point = center + radius * r * (1 - a)
        points.append(new_point)
    return np.array(points)


def flow_step(point, center, r):
    flow_speed = 0.033 * r
    direction = np.sign(center - point)
    return point + direction * flow_speed


# Modified Masterpiece Optimization Algorithm (MMOA) Starting Line No. 51
def Proposed(population, func, lb, ub, max_iter):
    ...
    for iteration in range(1, max_iter + 1):
        ...
        currentfit = fitness[iteration]
        r = bestfit / (worstfit + currentfit)
        points = np.vstack((points, create_quarter_points(center, radius, dim, r)))
        ...
        for i in range(len(best_candidates)):
            new_point = flow_step(best_candidates[i], center, r)
